...
var hideCustomGradebookColumn = require("./hideCustomGradebookColumn");
/**
...
const hideCustomGradebookColumnsByName = async (courseId, ...gradebookColumnNames) => {
  ...
  const matchingGradebookColumnID = customGradebookColumns.filter(({
    title
  }) => gradebookColumnNames.includes(title)).map(gradebookColumn => gradebookColumn.id);
  const response = Promise.all(matchingGradebookColumnID.map(gradebookId => hideCustomGradebookColumn(courseId, gradebookId)));
  return response;
};
module.exports = hideCustomGradebookColumnsByName;